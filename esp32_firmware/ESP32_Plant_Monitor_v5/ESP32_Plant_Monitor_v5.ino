 ...
 *   cross-check primaries per zone; averaged when within 10%, primary used
 *   with divergence warning when outside 10%; moisture_diverged flag in MQTT
 * - Dual DHT22: secondary on GPIO4 cross-checks primary; averaged when within
 *   2°C / 5% RH, primary used with divergence warning otherwise;
 *   dht_diverged flag in MQTT
 * - Freeze / unfreeze commands: MQTT "freeze" closes all valves and suspends
 *   all watering; "unfreeze" resumes normal operation
 * ──────────────────────────────────────────────────────────────────────────
 */
...
#define DHT_PIN   27    // DHT22 (primary) — air temp + humidity
#define DHT_PIN_2  4    // Second DHT22 — cross-checks primary for calibration drift detection
#define I2C_SDA   21    // BH1750 + MLX90614 + ADS1115 share this I2C bus
#define I2C_SCL   22
#define SD_CS      5
#define SD_MOSI   23
#define SD_MISO   19
...
// ═══════════════════════════════════════════════════════════════════
// SECONDARY MOISTURE SENSORS — ADS1115 I2C ADC (shares existing I2C bus)
// ADDR pin to GND → I2C address 0x48
// Channel 0 = Zone A secondary, channel 1 = Zone B secondary, etc.
// Wire secondary sensor signal pins to ADS1115 A0–A3 inputs
// MOISTURE_DRY and MOISTURE_WET calibration applies to both primary and secondary
#define DUAL_MOISTURE true   // set false to run with primary sensors only
...
struct SharedStatus {
  bool dht_ok       = true;
  bool dht_diverged = false;  // true when two DHT22s disagree (temp >2°C or humidity >5%)
  bool light_ok     = true;
  bool ir_ok        = true;
};
...
struct ZoneStatus {
  ...
  bool moisture_diverged = false;  // true when primary and secondary disagree by >10%
};
...
int readMoistureDual(int zone) {
#if !DUAL_MOISTURE
  // DUAL_MOISTURE disabled — behave identically to original readMoisture()
  ...
  zoneSensors[zone].moisture_diverged = false;
  return percent;
#else
  // ── Primary sensor (ESP32 ADC) ────────────────────────────────
  int rawA    = analogRead(MOISTURE_PINS[zone]);
  bool okA    = (rawA >= 100 && rawA <= 4000);
  ...
  // ── Cross-comparison ──────────────────────────────────────────
  if (okA && okB) {
    int diff = abs(pctA - pctB);
    if (diff <= 10) {
      // Both agree — average them
      zoneSensors[zone].moisture_ok       = true;
      zoneSensors[zone].moisture_diverged = false;
      return (pctA + pctB) / 2;
    } else {
      // Diverging — use primary, flag warning
      zoneSensors[zone].moisture_ok       = true;
      zoneSensors[zone].moisture_diverged = true;
      Serial.printf("MOISTURE DIVERGE [%s] — primary %d%%  secondary %d%%  diff %d%% — using primary\n",
        ZONE_IDS[zone], pctA, pctB, diff);
      return pctA;
    }
  } else if (okA) {
    // Only primary OK
    ...
    // Both failed
    zoneSensors[zone].moisture_ok       = false;
    zoneSensors[zone].moisture_diverged = false;
    return 0;
  }
#endif
...
void readSharedSensors() {
  ...
  if (ok1 && ok2) {
    float tDiff = abs(t1 - t2);
    float hDiff = abs(h1 - h2);
    if (tDiff <= 2.0 && hDiff <= 5.0) {
      // Both agree — average them
      sharedSensors.dht_ok       = true;
      sharedSensors.dht_diverged = false;
      sharedAirTemp  = (t1 + t2) / 2.0;
      sharedHumidity = (h1 + h2) / 2.0;
    } else {
      // Diverging — use primary, log warning with both readings
      sharedSensors.dht_ok       = true;
      sharedSensors.dht_diverged = true;
      sharedAirTemp  = t1;
      sharedHumidity = h1;
      Serial.printf("DHT DIVERGE — primary %.1f°C %.1f%%RH  secondary %.1f°C %.1f%%RH  diff %.1f°C %.1f%% — using primary\n",
        t1, h1, t2, h2, tDiff, hDiff);
    }
...
void runBootSelfTest() {
  ...
  if (isnan(t) || isnan(h))
    Serial.println("│    DHT22 (primary)   FAIL — no reading (check wiring GPIO27)");
  else
    Serial.printf( "│    DHT22 (primary)   OK   — %.1f°C  %.1f%%RH\n", t, h);

  // DHT22 secondary
  float t2 = dht2.readTemperature();
  float h2 = dht2.readHumidity();
  if (isnan(t2) || isnan(h2))
    Serial.println("│    DHT22 (secondary) FAIL — no reading (check wiring GPIO4)");
  else
  ...
  for (int z = 0; z < NUM_ZONES; z++) {
    ...
    if (rawP < 100 || rawP > 4000)
      Serial.printf("│    [%s] primary    FAIL — raw ADC %d (check wiring GPIO%d)\n",
        ZONE_IDS[z], rawP, MOISTURE_PINS[z]);
    else
      Serial.printf("│    [%s] primary    OK   — %d%% moisture (raw %d)\n",
        ZONE_IDS[z], pctP, rawP);
  ...
  for (int z = 0; z < NUM_ZONES; z++) {
    if (waitingForDripCheck[z] && now >= dripCheckTime[z]) {
      ...
      if (zoneSensors[z].moisture_ok) {
        int moistureAfter = readMoistureDual(z);
...
void controlZone(int zone, int moisture, float vpd, float lux) {
  // Freeze mode — suspend all watering
  if (freezeMode) {
    if (isWatering[zone]) stopDrip(zone, "freeze mode active");
    return;
  }
...
void publishZone(int zone, int moisture) {
  ...
  ok["moisture_diverged"] = zoneSensors[zone].moisture_diverged;
  ...
  for (int z = 0; z < NUM_ZONES; z++) {
    if (strcmp(topic, TOPIC_CMD[z]) == 0) {
      if      (strcmp(cmd,"water")==0)  startDrip(z,(unsigned long)(doc["duration"]|15)*1000,"remote");
        ...
        for (int z = 0; z < NUM_ZONES; z++) {
          if (isWatering[z]) stopDrip(z, "freeze command");
          digitalWrite(RELAY_PINS[z], LOW);
        }
        Serial.println("FREEZE MODE — watering suspended; all valves closed");
      }
      else if (strcmp(cmd, "unfreeze") == 0) {
        freezeMode = false;
        Serial.println("FREEZE MODE cleared — normal operation resumed");
      }
      return;
    }
  }
...
void loop() {
  ...
  if (now - lastReadingTime >= READING_INTERVAL) {
    ...
    Serial.println("\n──── SHARED SENSORS ──────────────────────────────");
    Serial.printf("Air:  %.1f°C  RH: %.1f%%  (DHT: %s%s)\n",
      sharedAirTemp, sharedHumidity,
      sharedSensors.dht_ok ? "ok" : "FAULT",
      sharedSensors.dht_diverged ? " DIVERGED" : "");
    ...
    if (freezeMode) Serial.println("*** FREEZE MODE ACTIVE — watering suspended ***");

    Serial.println("──── ZONES ───────────────────────────────────────");
    for (int z = 0; z < NUM_ZONES; z++) {
      int moisture = readMoistureDual(z);
      lastMoisture[z] = moisture;
      OperatingMode mode = determineModeForZone(z);
      Serial.printf("[%s]  moisture: %d%%%s  mode: %s  valve: %s  dripper: %s\n",
        ZONE_IDS[z], moisture,
        zoneSensors[z].moisture_diverged ? " (DIVERGED)" : "",
        modeNames[mode],

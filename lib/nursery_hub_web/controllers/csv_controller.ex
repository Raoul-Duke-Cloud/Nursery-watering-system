defmodule NurseryHubWeb.CsvController do
  ...
  # ── Combined zone CSV (readings + events) ─────────────────────────────────

  def download_combined(conn, %{"site_id" => site_id, "zone_id" => zone_id} = params) do
    ...
    with {:ok, from_date} <- Date.from_iso8601(from_str),
      ...
      csv =
        ["SENSOR READINGS,,,,,,,,,,"] ++
        [csv_header() | Enum.map(readings, &csv_row/1)] ++
        [""] ++
        ["WATERING EVENTS,,,,,,,,,,"] ++
        [events_header() | Enum.map(events, &events_row/1)]
        |> Enum.join("\n")
